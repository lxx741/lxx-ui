...
const meta: Meta<typeof LxxButton> = {
...
export const Default: Story & { args: { content: string } } = {
  ...
  render: (args) => ({
    components: { LxxButton },
    setup() {
      return { args }
    },
    template: container(
      `<lxx-button data-testid="story-test-btn" v-bind="args">{{args.content}}</lxx-button>`
    ),
  }),
...
export const Group: Story & { args: { content1: string; content2: string } } = {
  ...
  render: (args) => ({
    components: { LxxButton, LxxButtonGroup },
    setup() {
      return { args }
    },
    template: container(`
       <lxx-button-group :type="args.groupType" :size="args.groupSize" :disabled="args.groupDisabled">
         <lxx-button v-bind="args">{{args.content1}}</lxx-button>
         <lxx-button v-bind="args">{{args.content2}}</lxx-button>
       </lxx-button-group>
    `),
  }),
  play: async ({ canvasElement, args, step }) => {
